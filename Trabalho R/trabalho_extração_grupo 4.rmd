...
```
Após uma redução dos 10 000 mais diferencialmente expressos para apenas 50, é calculada a distância Euclidiana entre os *top_genes* e por fim são efetuados clusterings hierárquicos, representados através de dendogramas. Um com o método "single" onde são considerados os dois pontos mais próximos entre dois clusters para calcular a diferença entre estes, e outro com o método "average" onde é feita uma média de distância entre todos os pontos de 2 clusters resultando numa comparação mais balanceada. #ainda me falta intrepertar
